...
LuxMaxMesh::LuxMaxMesh(INode* p_node)
{
	...
	case GEOMOBJECT_CLASS_ID:
		...
		Matrix3 m3_nodeTransform = p_node->GetObjectTM(atoi(timestring));			// Get the node transform at current slider frame
...
void WriteMeshes(INode* p_node, FILE* s_pStream)
{
	...
	if (!p_node->IsRootNode() && !p_node->IsHidden())
	{
		...
		{
			//	LMX_ExportMesh $ "c:\\mymaxobject.txt"
			Mtl	*material; // = null;
			material = p_node->GetMtl();
			...
			*/
			...
			// Ignore Material Export for now
			/*
			if (lxmesh.material)
			{
