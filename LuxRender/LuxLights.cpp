...
void LuxMax::WriteSunSky(INode* n)
{
	Interval iv;
	iv.SetInfinite();
	LightState ls;
	...
	fprintf(s_pStream, "LightGroup \"%s\"\n", n->GetName());
	fprintf(s_pStream, "LightSource \"sunsky\" \"vector sundir\" [%s %s %s] \"float gain\" [%s] \"float relsize\" [1.000000] \"float turbidity\" [2.200000]\n", Format(sundir.x), Format(sundir.y), Format(sundir.z), Format(ls.intens));
	WritePortals(n_sceneRoot);
